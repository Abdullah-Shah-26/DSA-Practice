...
public:
  ...
  void dfs(int node, int parent, vector<bool> &vis, vector<vector<int>> &adj,
           vector<int> &insertionTime, vector<int> &lowestTime, vector<vector<int>> &bridges) {
    vis[node] = 1;
...
public:
  ...
  void dfs(int node, vector<vector<int>> &adj, vector<int> &tin,
           vector<int> &low, vector<int> &vis, vector<int> &inStack,
