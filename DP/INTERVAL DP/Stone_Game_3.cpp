...
//! Another Approach - Maximize Difference b/w Alice & BoB
//* We can compress dimension since we just have 2 players and they play
//alternatively
  ...
  int solve(int i, vector<int> &stones) {
    ...
    for (int x = 1; x <= min(3, n - i); x++) {
      ...
      ans = max(
          ans,
          sum - solve(i + x,
                      stones)); // MyGain - Opponent Response from next state
    }
