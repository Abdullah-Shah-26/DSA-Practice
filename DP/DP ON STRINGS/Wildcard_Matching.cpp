...
public:
  ...
  bool f(int i, int j, string &s, string &p){
    if(i == 0 && j == 0) return 1; // Both matched perfectly
    
    if(j == 0 && i > 0) return 0; // Pattern itself got exhausted

    if(i == 0 && j > 0){ // String got exhausted but if Pattern has all * it can make up 
      for(int k = 0; k < j; k++){
        if(p[k] != '*')
          return 0;
      }
