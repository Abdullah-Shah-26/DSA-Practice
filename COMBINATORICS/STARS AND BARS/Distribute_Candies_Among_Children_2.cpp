...
public:
  ...
  long long distributeCandies(int n, int limit) { return f(0, n, limit); }
};
...
public:
  long long distributeCandies(int n, int limit) {
    ...
    for (int i = firstMin; i <= firstMax; i++) {
      ...
      /*
      - If we give i to 1st child
      - we can choose to give 2ndMin to 2ndMax to 2nd child
      - remaining candies to 3rd child
      - so 2ndMax - 2ndMin + 1 distribution corresponds to this i
      */

      ways += secondMax - secondMin + 1;
    }
