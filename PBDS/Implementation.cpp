...
int main() {
  ...
  s.insert(2);
  s.insert(7);
  s.insert(2); // Ordered Set Contains Unique Values
  ...
  cout << "1 st Element : " << *s.find_by_order(1) << '\n';
  cout << "2 nd Element : " << *s.find_by_order(2) << '\n';
  nl();

  //* Finding No Smaller Than X
  cout << "No of Elements Less Than 6 : " << s.order_of_key(6) << '\n';
  cout << "No of Elements Less Than 11 : " << s.order_of_key(11) << '\n';
  cout << "No of Elements Less Than 1 : " << s.order_of_key(1) << '\n';
  nl();

  //* Lower Bound --- First Element >= X in set
