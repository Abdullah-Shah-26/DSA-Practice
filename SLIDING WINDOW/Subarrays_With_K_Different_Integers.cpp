...
public:
  int subarraysWithKDistinct(vector<int> &nums, int k) {
    ...
    // (part 1)count of subarr with atleeast k diff elements - (part 2)subarr
    // (k+1)atleast k+1 diff elem = total no of subarrays having exactly k diff
    // elements
...
public:
  int atmost(vector<int> &nums, int k) {
    ...
    int start = 0, end = 0, total = 0,
        count = 0; // stores the no of unique elem
    int n = nums.size();
  ...
  int subarraysWithKDistinct(vector<int> &nums, int k) {
    // Method - 3
    // no of subarrays having atmost(k) unique elements - no of subarrays having
    // atmost(k-1) unique elements = total no of subarr having exactly k unique
    // elem
