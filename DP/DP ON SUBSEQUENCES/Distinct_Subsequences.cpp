...
public:
  int f(int i, int j, string &s, string &t, vector<vector<int>> &dp) {
    ...
    if (s[i] == t[j]) {
      return dp[i][j] =
                 f(i - 1, j - 1, s, t, dp) +
                 f(i - 1, j, s, t, dp); // we are taking diff combinations of
                                        // same char at diffent positions
    }
...
public:
  int numDistinct(string s, string t) {
    ...
    // base case
    for (int i = 0; i <= n; i++)
      dp[i][0] = 1; // since j = 0 means s2 exhausted and we found all
                    // occurences of it in s1
    for (int j = 1; j <= m; j++)
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = 1; j <= m; j++) {
        ...
        if (s[i - 1] == t[j - 1]) {
          dp[i][j] = dp[i - 1][j - 1] +
                     dp[i - 1][j]; // we are taking diff combinations of same
                                   // char at diffent positions
  ...
  int numDistinct(string s, string t) {
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = m; j >= 1; j--) {
        ...
        // Char match
        if (s[i - 1] == t[j - 1]) {
          prev[j] = prev[j - 1] + prev[j]; // we are taking diff combinations of
                                           // same char at diffent positions
        }
      }
    }
