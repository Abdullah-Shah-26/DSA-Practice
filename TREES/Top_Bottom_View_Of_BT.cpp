...
public:
  ...
  void Tview(Node *root, int pos, vector<int> &ans, vector<int> &level,
             int l) // passed root ka level l = 0
  {
    // base case
    if (root == NULL)
      return;

    if (level[pos] > l) { // ans ke ander lv update tabhi hoga when curr lv
                          // chota ayega jo waha par already stored hay
      ans[pos] = root->data;
      level[pos] = l;
    }
  ...
  vector<int> topView(Node *root) {
    ...
    //
    Tview(root, -1 * l, ans, level,
          0); // Normalized ya shifted postion -1*(left most node ki pos)
    return ans;
  }
};
...
// vector<int> ans(r-l+1);// size of arr == right most node - left most node pos
// + 1 vector<bool>filled(r-l+1, 0); // to know that we have visited that node
// once and pos is filled in ans arr

// // 2 queues

// queue<Node*>q; // normal used for level order traversal
// queue<int>index; // to store index of Nodes  since we are checking if the pos
// is filled

// q.push(root);
// index.push(-l*1); // since we are shifing postions to match them with idx
// values
...
public:
  ...
  void Bview(Node *root, int pos, vector<int> &ans, vector<int> &level,
             int l) // passed root ka level l = 0
  {
    // base case
    if (root == NULL)
    ...
    if (level[pos] < l) { // ans ke ander lv update tabhi hoga when curr lv bada
                          // ayega jo waha par already stored hay
      // since its bottom view neeche ke level wale dhak dete upar walo ko same
      // pos raha to
      ans[pos] = root->data;
      level[pos] = l;
    }
